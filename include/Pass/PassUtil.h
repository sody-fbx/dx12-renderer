...
struct PipelineSet
{
	...
	void Create(ID3D12Device* device, ROOT_SIGNATURE_TYPE rootType, SHADERTYPE shaderType)
	{
		...
		case ROOT_SIGNATURE_TYPE_CBV:
			...
			break;
        case ROOT_SIGNATURE_TYPE_SHADOW:
            RootSignature.CreateWithShadow(device);
            break;
		default:
			break;
		}
...
struct ShadowMap
{
	...
	D3D12_GPU_DESCRIPTOR_HANDLE SRV() const { return SrvHeap.GetGPUHandle(0); }
    ID3D12DescriptorHeap* SRVHeap() const { return SrvHeap.Get(); }
